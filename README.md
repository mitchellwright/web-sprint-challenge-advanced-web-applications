...
1. Explain what a token is used for.
   A token is used to ensure that a request is coming from a source that has the proper privileges to access that resource. Generally it's used to ensure that someone is logged in before showing them data that should only be shown to logged in users.

2. What steps can you take in your web apps to keep your data secure?
   Make sure that proper information is protected by being behind authorization. Also ensure that your authorization is set up properly so that people can only access information when auth'ed into your service.

3. Describe how web servers work.
   A client (like a web browser) makes a call to the web server at a specific endpoint. The server will then run some code and return a response payload.
